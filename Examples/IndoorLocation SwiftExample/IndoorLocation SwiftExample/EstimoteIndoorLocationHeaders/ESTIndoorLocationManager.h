...
/**
 * TODO(All): this delegate is never called. Should be when position is nil with proper warm-up handling.
 */
- (void)indoorLocationManager:(ESTIndoorLocationManager *)manager
...
First you need to set `delegate` which will be receiving the updates and then start the manager for
...
Do no forget to stop the manager if you are not using location updates any more.
